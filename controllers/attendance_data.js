...
const response = await mongodb.getDatabase().db().collection('attendance').insertOne(attendance);
if(response.acknowledge){
    res.status(200).send();
}else{
    res.status(500).json(response.error || 'Some error occured while creating the attendance');
